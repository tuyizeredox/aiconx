...
const COMMUNITY_CATEGORIES = [ 
 ...
 export default function CommunityDetail() { 
   ...
   if (!community) return <div className="text-center py-20 text-slate-400 dark:text-ink-500">{t("common.loading")}</div>;
   ...
   return ( 
     <div className="max-w-2xl mx-auto px-4 py-4 lg:py-6"> 
       ...
       {/* Banner */} 
       <div className="rounded-2xl overflow-hidden mb-6 bg-white dark:bg-ink-800 border border-slate-100 dark:border-ink-700 shadow-sm">
         ...
         <div className="p-5 -mt-8 relative"> 
           <div className="w-16 h-16 rounded-2xl bg-white dark:bg-ink-700 shadow-lg border border-slate-100 dark:border-ink-600 flex items-center justify-center text-2xl mb-3 overflow-hidden">
             {community.icon_url ? (
           ...
           <div className="flex items-start justify-between gap-3"> 
             <div className="flex-1 min-w-0"> 
               ...
               <p className="text-sm text-slate-500 dark:text-ink-400 mb-3">{community.description}</p>
               <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-500 dark:text-ink-400">
       ...
       {isMember && ( 
         <Link to={`/CreatePost?community_id=${communityId}`}> 
           <div className="bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-700 rounded-2xl p-3 flex items-center gap-3 mb-5 hover:border-orange-300 dark:hover:border-orange-700 transition-colors cursor-pointer shadow-sm">
             ...
             <span className="text-sm text-slate-400 dark:text-ink-500 flex-1 min-w-0 truncate">{t("communities.shareWithCommunity")}</span>
       ...
       <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-4"> 
         <TabsList className="bg-white dark:bg-ink-800 border border-slate-100 dark:border-ink-700 w-full rounded-xl">
         ...
         <div className="space-y-4"> 
           ...
           <div className="space-y-4"> 
             {postsLoading 
               ...
               ? ( 
                 <div className="text-center py-16 bg-white dark:bg-ink-800 rounded-2xl border border-dashed border-slate-200 dark:border-ink-700">
                   <MessageSquare className="w-8 h-8 text-slate-300 dark:text-ink-600 mx-auto mb-2" />
                   <p className="text-sm text-slate-400 dark:text-ink-500">{t("communities.noPosts")}</p>
                   {isMember && ( 
                     <Link to={`/CreatePost?community_id=${communityId}`}> 
               ...
               : regularPosts.map(p => <PostCard key={p._id} post={p} currentUser={currentUser} feedPosts={regularPosts} />)}
           </div> 
         </div> 
       )} 

       {activeTab === "members" && ( 
         <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 divide-y divide-slate-50 dark:divide-ink-700 overflow-hidden shadow-sm">
           {members.length === 0 ? (
             <div className="text-center py-12 text-slate-400 dark:text-ink-500">{t("communities.noMembers")}</div>
           ) : (
             members.map(m => (
               <div key={m._id} className="flex items-center gap-3 p-4 hover:bg-slate-50 dark:hover:bg-ink-700/50 transition-colors">
                 <div className="w-9 h-9 rounded-full bg-gradient-to-br from-orange-400 to-orange-500 flex items-center justify-center text-white font-semibold text-sm shrink-0">
                   {m.member_username?.[0]?.toUpperCase()}
                 </div>
                 <div className="flex-1 min-w-0">
                   <p className="text-sm font-medium text-slate-800 dark:text-ink-200 truncate">@{m.member_username}</p>
                   <p className="text-xs text-slate-400 dark:text-ink-500">{t("communities.joinedOn", { date: new Date(m.joined_at).toLocaleDateString() })}</p>
                 </div>
                 <span className={`text-xs font-medium px-2 py-0.5 rounded-full capitalize ${m.role === "admin" ? "bg-orange-100 dark:bg-orange-950 text-orange-700 dark:text-orange-400" : m.role === "moderator" ? "bg-purple-100 dark:bg-purple-950 text-purple-700 dark:text-purple-400" : "bg-slate-100 dark:bg-ink-700 text-slate-600 dark:text-ink-300"}`}>
                   {m.role} 
                 </span> 
               </div> 
             )) 
       ...
       {activeTab === "about" && ( 
         <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-6 space-y-4 shadow-sm">
           <div>
             <h3 className="font-bold text-slate-900 dark:text-white mb-1">{t("communities.aboutThisCommunity")}</h3>
             <p className="text-sm text-slate-600 dark:text-ink-300 leading-relaxed">{community.description || t("communities.noDescription")}</p>
           </div>
           {community.rules && (
             <div>
               <h3 className="font-bold text-slate-900 dark:text-white mb-1">{t("communities.communityRules")}</h3>
               <p className="text-sm text-slate-600 dark:text-ink-300 whitespace-pre-wrap leading-relaxed">{community.rules}</p>
             </div>
           )}
           <div className="pt-4 border-t border-slate-100 dark:border-ink-700 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-500 dark:text-ink-400">
