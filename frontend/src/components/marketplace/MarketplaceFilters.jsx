...
function Section({ icon: Icon, title, children, action }) {
  return (
    <div className="py-4 border-b border-slate-100 dark:border-ink-800 last:border-b-0">
      <div className="flex items-center justify-between gap-2 mb-3">
        <h3 className="text-[11px] font-black uppercase tracking-wider text-slate-400 dark:text-ink-500 flex items-center gap-1.5">
    ...
    <div className="divide-y-0">
      <div className="flex items-center justify-between gap-2 pb-3 border-b border-slate-100 dark:border-ink-800">
        ...
        {activeCount > 0 && (
          <button
            onClick={onClear}
            className="text-xs font-semibold text-slate-500 dark:text-ink-400 hover:text-orange-600 transition-colors flex items-center gap-1"
          >
      ...
      <Section icon={MapPin} title={t("shop.nearMe")}>
        <div className="flex items-center justify-between gap-3 mb-2">
          <p className="text-sm font-medium text-slate-700 dark:text-ink-300">{t("shop.shopNearMe")}</p>
        ...
        <p className={`text-xs leading-relaxed flex items-start gap-1.5 ${
          nearbyFailed ? "text-amber-600 dark:text-amber-500" : "text-slate-500 dark:text-ink-400"
        }`}>
        ...
        {nearby.enabled && nearby.status === "ready" && (
          <div className="mt-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-slate-500 dark:text-ink-400">{t("shop.searchRadius")}</span>
            ...
            <Slider
              ...
              step={1}
            />
            <div className="flex justify-between mt-1.5 text-[10px] text-slate-400 dark:text-ink-500">
              ...
              <button
                ...
                className={`w-full flex items-center justify-between gap-2 px-2.5 py-2 rounded-xl text-sm transition-colors ${
                  selected
                    ...
                    : count
                      ? "text-slate-600 dark:text-ink-300 hover:bg-slate-50 dark:hover:bg-ink-800"
                      : "text-slate-300 dark:text-ink-600 hover:bg-slate-50 dark:hover:bg-ink-800"
                }`}
              >
                <span className="truncate">{t(`shop.category_${key}`)}</span>
                {count > 0 && (
                  <span className="text-[10px] font-bold text-slate-400 dark:text-ink-500 shrink-0">{count}</span>
                )}
              </button>
            );
          })}
      ...
      <Section icon={Tag} title={t("shop.priceRange")}>
        <div className="flex items-center gap-2">
          ...
          />
          <span className="text-slate-300 dark:text-ink-600 shrink-0">–</span>
          <Input
            type="number"
            inputMode="numeric"
            min={0}
            ...
            return (
              ...
              >
                {Array(5).fill(0).map((_, i) => (
                  <Star
                    key={i}
                    className={`w-3.5 h-3.5 ${i < value ? "fill-amber-400 text-amber-400" : "text-slate-200 dark:text-ink-700"}`}
                  />
                ))}
                <span className="text-xs text-slate-500 dark:text-ink-400 ml-1">{t("shop.andUp")}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-sm text-slate-700 dark:text-ink-300">{t("shop.inStockOnly")}</p>
              <p className="text-[11px] text-slate-400 dark:text-ink-500">{t("shop.inStockHint")}</p>
            </div>
            <Switch checked={values.inStock} onCheckedChange={(v) => onChange({ inStock: v })} />
          </div>
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-sm text-slate-700 dark:text-ink-300">{t("shop.onSaleOnly")}</p>
              <p className="text-[11px] text-slate-400 dark:text-ink-500">{t("shop.onSaleHint")}</p>
            </div>
