...
function TrendingCard({ product, t, navigate }) {
  ...
  return (
    ...
    <Link
      ...
      className="group block w-[44vw] sm:w-auto shrink-0 snap-start bg-white dark:bg-ink-900 rounded-2xl overflow-hidden border border-slate-100 dark:border-ink-800 active:scale-[0.98] sm:hover:shadow-xl sm:hover:shadow-slate-100 dark:sm:hover:shadow-black/30 transition-all duration-300"
    >
      <div className="relative aspect-square overflow-hidden">
        ...
        <button
          ...
          className="absolute top-2.5 right-2.5 w-9 h-9 rounded-full bg-white/90 dark:bg-ink-800/90 hover:bg-white dark:hover:bg-ink-800 flex items-center justify-center shadow-md backdrop-blur-sm transition-colors active:scale-90"
        >
          <Heart className="w-4 h-4 text-slate-600 dark:text-ink-300" />
        </button>
      </div>
      <div className="p-3">
        <p className="flex items-center gap-1 text-[11px] text-slate-400 dark:text-ink-500 font-medium mb-1">
          <Store className="w-3 h-3 shrink-0" />
          <span className="truncate">{product.store_name || t("profile.store")}</span>
        </p>
        <h3 className="text-sm font-semibold text-slate-900 dark:text-ink-100 line-clamp-2 leading-tight mb-1.5">{product.title}</h3>
        ...
        <p className="text-base font-bold text-slate-900 dark:text-ink-100 mb-2">{formatCurrency(product.price)}</p>
        <button
