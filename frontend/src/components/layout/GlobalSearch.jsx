...
export default function GlobalSearch() {
  ...
  return (
    <div ref={ref} className="relative w-full max-w-xs">
      <div className="flex items-center gap-2 bg-slate-100 dark:bg-ink-800 rounded-xl px-3 py-2 transition-all focus-within:bg-white dark:focus-within:bg-ink-800 focus-within:ring-2 focus-within:ring-orange-300 focus-within:shadow-sm">
        ...
        <input
          ...
          className="flex-1 bg-transparent text-sm text-slate-700 dark:text-ink-300 placeholder:text-slate-400 outline-none"
        />
        {query && (
      ...
      <AnimatePresence>
        {open && query.length >= 2 && (
          <motion.div
            ...
            className="absolute top-full mt-2 left-0 right-0 bg-white dark:bg-ink-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-ink-700 z-50 overflow-hidden min-w-[320px]"
          >
            {/* Category filter tabs */}
            <div className="flex gap-1 p-2 border-b border-slate-100 dark:border-ink-700 overflow-x-auto hide-scrollbar">
              {CATEGORIES.map(cat => (
                <button
                  key={cat}
                  onClick={() => setActiveCategory(cat)}
                  className={`shrink-0 text-[11px] font-semibold px-3 py-1 rounded-lg capitalize transition-colors ${
                    activeCategory === cat ? "bg-orange-600 text-white" : "bg-slate-100 dark:bg-ink-700 text-slate-500 dark:text-ink-400 hover:bg-slate-200 dark:hover:bg-ink-600"
                  }`}
                >
                  {cat}
                </button>
            ...
            <div className="max-h-[400px] overflow-y-auto">
              ...
              {!isLoading && !hasResults && (
                <div className="py-10 text-center">
                  <Search className="w-8 h-8 text-slate-200 dark:text-ink-600 mx-auto mb-2" />
                  <p className="text-sm text-slate-400 dark:text-ink-500">No results for "<span className="font-medium">{query}</span>"</p>
                </div>
              )}
              ...
              {!isLoading && showProducts && products.length > 0 && (
                <div>
                  <p className="px-3 py-1.5 text-[10px] font-bold text-slate-400 dark:text-ink-500 uppercase tracking-wider border-b border-slate-50 dark:border-ink-700">Products</p>
                  {products.map(p => (
                    <Link
                      key={p.id}
                      to={createPageUrl("ProductDetail") + `?id=${p.id}`}
                      onClick={handleResultClick}
                      className="flex items-center gap-3 px-3 py-2.5 hover:bg-slate-50 dark:hover:bg-ink-700 transition-colors"
                    >
                      <div className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-ink-700 overflow-hidden shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400 dark:text-ink-500">{p.store_name || "Store"} · <span className="font-semibold text-orange-600">${p.price}</span></p>
                      </div>
                    </Link>
                  ))}
                </div>
              ...
              {!isLoading && showStores && stores.length > 0 && (
                <div>
                  <p className="px-3 py-1.5 text-[10px] font-bold text-slate-400 dark:text-ink-500 uppercase tracking-wider border-b border-slate-50 dark:border-ink-700 border-t border-slate-100 dark:border-ink-700">Stores</p>
                  {stores.map(s => (
                    <Link
                      key={s.id || s._id}
                      to={storeUrl(s)}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400 dark:text-ink-500 capitalize">{s.category} · {s.product_count || 0} products</p>
                      </div>
              ...
              {!isLoading && showUsers && users.length > 0 && (
                <div>
                  <p className="px-3 py-1.5 text-[10px] font-bold text-slate-400 dark:text-ink-500 uppercase tracking-wider border-b border-slate-50 dark:border-ink-700 border-t border-slate-100 dark:border-ink-700">People</p>
                  {users.map(u => (
                    <Link
                      key={u.id || u._id}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400 dark:text-ink-500 truncate">@{u.username || u.display_name?.replace(/\s+/g, '_').toLowerCase()}</p>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            {/* Footer hint */}
            <div className="px-3 py-2 border-t border-slate-100 dark:border-ink-700 bg-slate-50 dark:bg-ink-900">
              <p className="text-[10px] text-slate-400 dark:text-ink-500">Press <kbd className="bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-700 rounded px-1 text-[10px]">Enter</kbd> to search all products</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
