...
export default function NotificationBell({ userEmail }) {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <Bell className="w-[21px] h-[21px] text-slate-600 dark:text-ink-300" />
        {unread > 0 && (
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-full mt-2 w-[min(20rem,calc(100vw-1.5rem))] max-w-[20rem] bg-white dark:bg-ink-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-ink-700 z-50 overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-ink-700">
              <div className="flex items-center gap-2">
                <Bell className="w-4 h-4 text-slate-700 dark:text-ink-300" />
                <span className="text-sm font-bold text-slate-900 dark:text-white">Notifications</span>
                {unread > 0 && (
                  <span className="text-xs bg-orange-100 dark:bg-orange-950 text-orange-700 dark:text-orange-400 font-semibold rounded-full px-2 py-0.5">{unread} new</span>
                )}
              </div>
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => setOpen(false)} className="ml-2 p-1 hover:bg-slate-100 dark:hover:bg-ink-700 rounded-lg">
                  <X className="w-3.5 h-3.5 text-slate-400 dark:text-ink-500" />
                </button>
              </div>
            </div>
            ...
            <div className="max-h-96 overflow-y-auto">
              {notifications.length === 0 ? (
                <div className="py-10 text-center">
                  <Bell className="w-8 h-8 text-slate-200 dark:text-ink-700 mx-auto mb-2" />
                  <p className="text-sm text-slate-400 dark:text-ink-500">All caught up!</p>
                </div>
              ) : (
                notifications.map(notif => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-start gap-3 px-4 py-3 hover:bg-slate-50 dark:hover:bg-ink-800 transition-colors text-left border-b border-slate-50 dark:border-ink-700/50 last:border-0 ${!notif.is_read ? "bg-orange-50/40 dark:bg-orange-950/40" : ""}`}
                    >
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${cfg.color}`}>
                        <Icon className="w-3.5 h-3.5" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-slate-800 dark:text-white leading-snug">{notif.title}</p>
                        {notif.body && <p className="text-[11px] text-slate-500 dark:text-ink-400 mt-0.5 line-clamp-2">{notif.body}</p>}
                        <p className="text-[10px] text-slate-400 dark:text-ink-500 mt-1">
            ...
            {/* Footer */}
            <div className="px-4 py-2 border-t border-slate-100 dark:border-ink-700">
              <Link
