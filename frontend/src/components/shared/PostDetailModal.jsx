  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        className={`w-full h-full max-w-4xl sm:max-w-5xl md:max-w-6xl lg:max-w-7xl p-0 gap-0 overflow-hidden border-0 rounded-none sm:rounded-2xl bg-slate-50 dark:bg-[#0a0a0c] ${contentClassName}`}
      >
        ...
        <div className="flex flex-col lg:flex-row h-full max-h-[100vh] overflow-y-auto lg:overflow-hidden custom-scrollbar">
            ...
            <div className="sticky top-0 z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 pt-[max(1rem,env(safe-area-inset-top))] px-4 pb-4 flex items-center justify-between">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-bold text-slate-900 dark:text-slate-100 truncate">{post.author_name || "User"}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 truncate">@{post.author_username}</p>
                </div>
              </div>
              <button
                onClick={() => onOpenChange(false)}
                aria-label={t("common.close")}
                className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-500 transition-colors shrink-0"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
          ...
          <div className="w-full lg:w-[400px] xl:w-[450px] border-l border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col lg:max-h-full lg:min-h-0">
            ...
            <CommentComposer
              postId={postId}
              currentUser={currentUser}
              className="sticky bottom-0 lg:static bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl px-4 pt-4 pb-[max(1rem,env(safe-area-inset-bottom))] border-t border-slate-100 dark:border-slate-800"
            />
          </div>
        </div>
      </DialogContent>
