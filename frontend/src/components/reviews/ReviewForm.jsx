...
export default function ReviewForm({ productId, storeId, currentUser, onClose }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-200 dark:border-ink-700 p-5">
      ...
      <div className="flex items-center gap-1 mb-4">
        {[1, 2, 3, 4, 5].map(s => (
          <button
            ...
            onClick={() => setRating(s)}
          >
            <Star className={`w-8 h-8 transition-colors ${s <= (hoverRating || rating) ? "fill-amber-400 text-amber-400" : "text-slate-200 dark:text-ink-700"}`} />
          </button>
        ))}
        {rating > 0 && <span className="text-sm text-slate-500 dark:text-ink-400 ml-2">{["", t("product.ratingPoor"), t("product.ratingFair"), t("product.ratingGood"), t("product.ratingVeryGood"), t("product.ratingExcellent")][rating]}</span>}
      </div>
      ...
      {/* Media Upload */}
      <div className="mb-4">
        <p className="text-xs font-medium text-slate-600 dark:text-ink-300 mb-2 flex items-center gap-1.5">
          <Camera className="w-3.5 h-3.5" /> {t("product.addPhotosVideos")}
        </p>
        <div className="flex gap-2 flex-wrap">
          {previews.map((prev, i) => (
            <div key={`preview-${i}-${prev}`} className="relative w-16 h-16 rounded-xl overflow-hidden border border-slate-200 dark:border-ink-700">
          ...
          {mediaFiles.length < 5 && (
            <label className="w-16 h-16 rounded-xl border-2 border-dashed border-slate-200 dark:border-ink-700 flex flex-col items-center justify-center cursor-pointer hover:border-orange-400 transition-colors">
              <Upload className="w-5 h-5 text-slate-400 dark:text-ink-500" />
              <span className="text-[9px] text-slate-400 dark:text-ink-500 mt-0.5">{t("common.upload")}</span>
