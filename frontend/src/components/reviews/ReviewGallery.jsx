  ...
  return (
    <div className="mt-6">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-slate-400 dark:text-ink-500 font-medium">{t("product.photosAndVideos", { count: allMedia.length })}</span>
      </div>
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-2">
        {allMedia.slice(0, 10).map((media, i) => (
          <motion.button
            ...
            className="relative aspect-square rounded-xl overflow-hidden bg-slate-100 dark:bg-ink-800 group"
          >
            {isVideo(media.url) ? (
              <div className="w-full h-full bg-slate-200 dark:bg-ink-700 flex items-center justify-center">
                <Play className="w-8 h-8 text-slate-500 dark:text-ink-400" />
              </div>
            ) : (
        ...
        {allMedia.length > 10 && (
          <button
            ...
            className="aspect-square rounded-xl bg-slate-100 dark:bg-ink-800 flex items-center justify-center text-sm font-bold text-slate-600 dark:text-ink-300 hover:bg-slate-200 dark:hover:bg-ink-700 transition-colors"
          >
            +{allMedia.length - 10}
          </button>
        )}
