...
export default function SizeInput({ sizes = [], onChange }) {
  ...
  return (
    <div>
      ...
      <p className="text-xs text-slate-400 dark:text-ink-500 mb-2">{t("store.productSizesHint")}</p>
      ...
      {sizes.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-2.5">
          {sizes.map((s, i) => (
            <span
              key={`${s}-${i}`}
              className="inline-flex items-center gap-1.5 pl-2.5 pr-1 py-1 rounded-full bg-slate-100 dark:bg-ink-700 text-xs font-medium text-slate-700 dark:text-ink-200"
            >
              {s}
              <button
                type="button"
