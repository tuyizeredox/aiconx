...
export default function BusinessProductPost({ store, storeName, products = [], distanceKm = null }) {
  ...
  const Header = (
    <div className="flex items-center gap-3 px-4 pt-4 pb-3">
      <Link to={shopHref} className="w-10 h-10 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 shrink-0 flex items-center justify-center">
        {store?.logo_url
      ...
      <article className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
        ...
        <div className="p-4">
          <Link to={createPageUrl("ProductDetail") + "?id=" + id} onClick={track} className="block">
            ...
            <p className={`text-[12px] mt-1 ${soldOut ? "text-slate-400 dark:text-slate-500" : "text-green-600 dark:text-green-500"}`}>
          ...
          <Link
            ...
            onClick={track}
            className="mt-4 flex items-center justify-center h-12 rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-[15px] font-bold hover:opacity-90 transition-opacity"
          >
            {t("home.viewProduct")}
          </Link>
        </div>
  ...
  return (
    <article className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden">
      ...
      <div className="p-4 pt-3">
        <Link
          to={shopHref}
          className="flex items-center justify-center gap-1 h-11 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-900 dark:text-white text-[14px] font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
        >
