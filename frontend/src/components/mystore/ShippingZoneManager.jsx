...
function ZoneForm({ initial = BLANK_ZONE, onSave, onCancel, saving }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="bg-slate-50 dark:bg-ink-800 rounded-2xl border border-slate-200 dark:border-ink-700 p-4 space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2">
          <label className="text-xs text-slate-500 dark:text-ink-400 font-medium block mb-1">{t("store.zoneName")}</label>
          <Input
        ...
        <div>
          <label className="text-xs text-slate-500 dark:text-ink-400 font-medium block mb-1">{t("store.flatRateUSD")}</label>
          <Input
            type="number"
        ...
        <div>
          <label className="text-xs text-slate-500 dark:text-ink-400 font-medium block mb-1">{t("store.freeAboveLabel")}</label>
          <Input
            type="number"
        ...
        <div>
          <label className="text-xs text-slate-500 dark:text-ink-400 font-medium block mb-1">{t("store.minDays")}</label>
          <Input
            type="number"
        ...
        <div>
          <label className="text-xs text-slate-500 dark:text-ink-400 font-medium block mb-1">{t("store.maxDays")}</label>
          <Input
            type="number"
      ...
      <div>
        <label className="text-xs text-slate-500 dark:text-ink-400 font-medium block mb-2">{t("store.countriesRegions")}</label>
        <div className="flex flex-wrap gap-2">
          {COUNTRY_PRESETS.map(c => (
            <button
              ...
              className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all border ${form.countries.includes(c.code) ? "bg-orange-600 text-white border-orange-600" : "bg-white dark:bg-ink-700 text-slate-600 dark:text-ink-300 border-slate-200 dark:border-ink-600 hover:border-slate-300 dark:hover:border-ink-500"}`}
            >
              {c.label}
            </button>
          ))}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-ink-400 mt-0.5">{t("store.shippingZonesDesc")}</p>
        </div>
        <Button 
          onClick={() => {
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 border-2 border-dashed border-slate-200 dark:border-ink-700 rounded-2xl">
          <Globe className="w-8 h-8 text-slate-300 dark:text-ink-600 mx-auto mb-2" />
          <p className="text-sm text-slate-500 dark:text-ink-400 font-medium">{t("store.noShippingZones")}</p>
          <p className="text-xs text-slate-400 dark:text-ink-500">{t("store.addZonesDesc")}</p>
        </div>
      ) : (
        <div className="space-y-3">
          {zones.map(zone => (
            <motion.div key={zone._id || zone.id} layout className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 overflow-hidden">
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${zone.is_active ? "bg-orange-50 dark:bg-orange-950" : "bg-slate-100 dark:bg-ink-700"}`}>
                        <MapPin className={`w-4 h-4 ${zone.is_active ? "text-orange-500" : "text-slate-400"}`} />
                      </div>
                      <div>
                        <p className={`text-sm font-semibold ${zone.is_active ? "text-slate-900 dark:text-white" : "text-slate-400 dark:text-ink-500 line-through"}`}>{zone.zone_name}</p>
                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                          <span className="text-xs font-bold text-green-600">${(Number(zone.flat_rate) || 0).toFixed(2)} {t("store.flatRate")}</span>
                          {(Number(zone.free_above) || 0) > 0 && <span className="text-xs text-slate-400 dark:text-ink-500">· {t("store.freeOver", { amount: zone.free_above })}</span>}
                          <span className="text-xs text-slate-400 dark:text-ink-500">· {t("store.estimatedDays", { min: zone.estimated_days_min, max: zone.estimated_days_max })}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        ...
                        className={`w-8 h-5 rounded-full transition-colors relative ${zone.is_active ? "bg-orange-500" : "bg-slate-200 dark:bg-ink-600"}`}
                      >
                        <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all ${zone.is_active ? "left-3.5" : "left-0.5"}`} />
                      </button>
                      <button onClick={() => setEditId(zone._id || zone.id)} className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-ink-700 text-slate-400 hover:text-slate-600 dark:hover:text-ink-300 transition-colors">
      ...
      <div className={`mt-8 p-5 rounded-2xl border-2 border-dashed transition-all ${isElite ? "bg-amber-50/30 dark:bg-amber-950/30 border-amber-100 dark:border-amber-900" : "bg-slate-50 dark:bg-ink-800 border-slate-200 dark:border-ink-700"}`}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <Badge className={isElite ? "bg-amber-500 text-white border-0" : "bg-slate-200 dark:bg-ink-700 text-slate-500 dark:text-ink-400 border-0"}>Elite</Badge>
            </div>
            <p className="text-xs text-slate-500 dark:text-ink-400 leading-relaxed max-w-md">
        ...
        {!isElite ? (
          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-ink-700 flex items-center justify-between">
            <span className="text-[11px] text-slate-400 dark:text-ink-500 font-medium">{t("store.upgradeForLiveRates")}</span>
          ...
          <div className="mt-4 p-3 bg-white dark:bg-ink-800 rounded-xl border border-amber-100 dark:border-amber-900 text-center">
