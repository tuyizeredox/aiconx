...
const CustomTooltip = ({ active, payload, label, prefix = "" }) => {
  if (active && payload?.length) {
    return (
      <div className="bg-white dark:bg-ink-800 border border-slate-100 dark:border-ink-700 rounded-xl shadow-lg p-3">
        <p className="text-xs text-slate-500 dark:text-ink-400 mb-1">{label}</p>
        {payload.map((p, i) => (
  ...
  return (
    <div className="space-y-6">
        ...
        >
          <div className="flex items-center gap-3 text-center md:text-left">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-ink-400">{t("store.upgradeForInsights")}</p>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5">
        ...
        <p className="text-xs text-slate-400 dark:text-ink-500 mb-4">{t("store.dailyRevenueDesc")}</p>
        {dailyRevenue.every(d => d.revenue === 0) ? (
          <div className="h-[200px] flex items-center justify-center text-sm text-slate-400 dark:text-ink-500">{t("store.noOrderLast7Days")}</div>
        ) : (
      ...
      <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5">
        ...
        <p className="text-xs text-slate-400 dark:text-ink-500 mb-4">{t("store.unitsSoldAllOrders")}</p>
        {topProducts.length === 0 || topProducts.every(p => p.sales === 0) ? (
          <div className="h-[180px] flex items-center justify-center text-sm text-slate-400 dark:text-ink-500">{t("store.noSalesDataYet")}</div>
        ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5">
          ...
          <p className="text-xs text-slate-400 dark:text-ink-500 mb-4">{t("store.orderStatusDesc")}</p>
          {orderStatusData.length === 0 ? (
            <div className="h-[140px] flex items-center justify-center text-sm text-slate-400 dark:text-ink-500">{t("store.noOrdersYetAnalytics")}</div>
          ) : (
              ...
              <div className="space-y-2">
                {orderStatusData.map((d, i) => (
                  <div key={d.name} className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-600 dark:text-ink-300">{d.name}</span>
        ...
        <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5">
          ...
          {lowStockProducts.length === 0 ? (
            <div className="h-[100px] flex items-center justify-center text-sm text-slate-400 dark:text-ink-500">{t("store.wellStocked")}</div>
          ) : (
            <div className="space-y-2">
              {lowStockProducts.map(p => (
                <div key={p._id || p.id} className="flex items-center justify-between py-2 border-b border-slate-50 dark:border-ink-700 last:border-0">
