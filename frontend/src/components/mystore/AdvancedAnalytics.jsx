...
const CustomTooltip = ({ active, payload, label, prefix = "" }) => {
  if (active && payload?.length) {
    return (
      <div className="bg-white dark:bg-ink-800 border border-slate-100 dark:border-ink-700 rounded-xl shadow-lg p-3 text-xs">
        <p className="text-slate-500 dark:text-ink-400 mb-1 font-medium">{label}</p>
        {payload.map((p, i) => (
...
function KpiCard({ icon: Icon, label, value, change, color, sub }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-4">
      ...
      <p className="text-xs text-slate-500 dark:text-ink-400 mt-0.5">{label}</p>
      {sub && <p className="text-[10px] text-slate-400 dark:text-ink-500 mt-0.5">{sub}</p>}
    </motion.div>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5">
        ...
        <p className="text-xs text-slate-400 dark:text-ink-500 mb-4">{t("store.revenueTrendDesc")}</p>
        {revenueOrderData.every(d => d.revenue === 0) ? (
          <div className="h-[200px] flex items-center justify-center text-sm text-slate-400 dark:text-ink-500">{t("store.noOrderHistory")}</div>
        ) : (
      ...
      <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5">
        ...
        <p className="text-xs text-slate-400 dark:text-ink-500 mb-3">{t("store.productPerfDesc")}</p>
        {productPerf.length === 0 ? (
          <div className="py-8 text-center text-sm text-slate-400 dark:text-ink-500">{t("store.noSalesDataYet")}</div>
        ) : (
              ...
              <thead>
                <tr className="border-b border-slate-100 dark:border-ink-700">
                  {[t("store.productTableProduct"), t("store.productTableUnits"), t("store.productTableRevenue")].map(h => (
                    <th key={h} className="text-left pb-2 text-slate-500 dark:text-ink-400 font-medium pr-3 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {productPerf.map((p, i) => (
                  <motion.tr
                    ...
                    className="border-b border-slate-50 dark:border-ink-700 hover:bg-slate-50 dark:hover:bg-ink-700/50 transition-colors"
                  >
                    <td className="py-2.5 pr-3 font-semibold text-slate-800 dark:text-ink-200">{p.name}</td>
                    <td className="py-2.5 pr-3 text-slate-600 dark:text-ink-300">{p.purchases.toLocaleString()}</td>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5">
          ...
          <p className="text-xs text-slate-400 dark:text-ink-500 mb-4">{t("store.revenueByCategoryDesc")}</p>
          {categoryRevenue.length === 0 ? (
            <div className="h-[160px] flex items-center justify-center text-sm text-slate-400 dark:text-ink-500">{t("store.noCategoryData")}</div>
          ) : (
        ...
        <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5">
          ...
          <p className="text-xs text-slate-400 dark:text-ink-500 mb-4">{t("store.deliveryMethodsDesc")}</p>
          {deliveryMethodData.length === 0 ? (
            <div className="h-[160px] flex items-center justify-center text-sm text-slate-400 dark:text-ink-500">{t("store.noOrderDataYet")}</div>
          ) : (
              ...
              <div className="flex-1 space-y-2.5">
                {deliveryMethodData.map(d => (
                  <div key={d.name} className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-600 dark:text-ink-300 flex-1">{d.name}</span>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5">
          ...
          <p className="text-xs text-slate-400 dark:text-ink-500 mb-4">{t("store.orderStatusPipelineDesc")}</p>
          {orderStatusData.length === 0 ? (
            <div className="py-8 text-center text-sm text-slate-400 dark:text-ink-500">{t("store.noOrdersYetAnalytics")}</div>
          ) : (
            <div className="space-y-2">
                ...
                return (
                  <div key={s.name}>
                    <div className="flex justify-between text-xs mb-0.5">
                      <span className="text-slate-600 dark:text-ink-300">{s.name}</span>
                      <span className="font-semibold text-slate-900 dark:text-white">{s.value} <span className="text-slate-400 dark:text-ink-500 font-normal">({pct}%)</span></span>
                    </div>
              ...
              <p className="text-xs text-slate-400 dark:text-ink-500 mt-3">{t("store.orderCompletionRate")} <span className="text-orange-600 font-semibold">{completionRate}%</span></p>
            </div>
          )}
        </div>

        <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5 relative overflow-hidden">
          {!isElite && (
            <div className="absolute inset-0 bg-white/80 dark:bg-ink-800/80 backdrop-blur-[1px] z-10 flex flex-col items-center justify-center p-6 text-center">
              ...
              <p className="text-[11px] text-slate-500 dark:text-ink-400 mb-4 max-w-[180px]">{t("store.upgradeForLocation")}</p>
          ...
          <p className="text-xs text-slate-400 dark:text-ink-500 mb-3">{t("store.topShippingCountriesDesc")}</p>
          {topCountries.length === 0 ? (
            <div className="py-8 text-center text-sm text-slate-400 dark:text-ink-500">{t("store.noShippingData")}</div>
          ) : (
            <div className="space-y-2">
              {topCountries.map((loc, i) => (
                <div key={loc.country} className="flex items-center gap-2">
                  <span className="text-xs font-bold text-slate-400 dark:text-ink-500 w-4">{i + 1}</span>
                  <span className="text-xs text-slate-700 dark:text-ink-300 flex-1">{loc.country}</span>
                  <div className="w-20 h-1.5 bg-slate-100 dark:bg-ink-700 rounded-full overflow-hidden">
      ...
      <div className="bg-white dark:bg-ink-800 rounded-2xl border border-slate-100 dark:border-ink-700 p-5">
        ...
        <p className="text-xs text-slate-400 dark:text-ink-500 mb-4">{t("store.lowStockDesc")}</p>
        {lowStockProducts.length === 0 ? (
          <p className="text-sm text-slate-400 dark:text-ink-500">{t("store.wellStocked")}</p>
        ) : (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {lowStockProducts.map(p => (
              <div key={p._id || p.id} className="flex items-center justify-between p-3 rounded-xl bg-amber-50 dark:bg-amber-950 border border-amber-100 dark:border-amber-900">
                <span className="text-xs font-medium text-slate-700 dark:text-ink-300 truncate max-w-[60%]">{p.title}</span>
