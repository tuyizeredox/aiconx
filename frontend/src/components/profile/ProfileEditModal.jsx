  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-sm rounded-2xl">
        ...
        {/* Banner */}
        <div className="relative h-24 rounded-xl overflow-hidden mb-2 bg-slate-100 dark:bg-ink-800 group">
          {bannerUrl ? (
        ...
        <div className="flex justify-center -mt-10 relative z-10 mb-2">
          <label className="relative cursor-pointer group">
            <div className="w-20 h-20 rounded-full overflow-hidden bg-gradient-to-br from-orange-400 to-orange-500 flex items-center justify-center ring-4 ring-white dark:ring-ink-800 shadow-lg">
              <AvatarImg
                src={avatarUrl}
        ...
        <div className="space-y-3">
          <div>
            <label className="text-xs font-semibold text-slate-600 dark:text-ink-300 block mb-1">Display Name</label>
            <Input value={displayName} onChange={e => setDisplayName(e.target.value)} className="rounded-xl" placeholder="Your display name" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-600 dark:text-ink-300 block mb-1">Bio</label>
            <textarea
              value={bio}
            ...
            />
            <p className="text-right text-[10px] text-slate-400 dark:text-ink-500">{bio.length}/160</p>
          </div>
        </div>
