...
export default function OrderStatusCard({ orderId, isVendor }) {
  ...
  return (
    <div className="mt-2 bg-white dark:bg-ink-800 border border-slate-200 dark:border-ink-700 rounded-2xl overflow-hidden shadow-sm">
      <div className="flex items-center justify-between px-3 py-2 bg-slate-50 dark:bg-ink-900 border-b border-slate-100 dark:border-ink-700">
        <div className="flex items-center gap-1.5">
          <Package className="w-3.5 h-3.5 text-slate-500 dark:text-ink-400" />
          <span className="text-xs font-semibold text-slate-700 dark:text-ink-200">Order #{order.id?.slice(-6)}</span>
        </div>
      ...
      <div className="px-3 pt-3 pb-1">
        <div className="flex items-center gap-0.5">
          {STEPS.map((step, i) => (
            <React.Fragment key={step.key}>
              <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 border ${i <= currentStep ? step.color : "bg-slate-100 dark:bg-ink-700 border-slate-200 dark:border-ink-600 text-slate-300 dark:text-ink-500"}`}>
                <step.icon className="w-2.5 h-2.5" />
              </div>
              {i < STEPS.length - 1 && (
                <div className={`flex-1 h-0.5 ${i < currentStep ? "bg-orange-400" : "bg-slate-200 dark:bg-ink-600"}`} />
              )}
            </React.Fragment>
          ))}
        </div>
        <div className="flex justify-between mt-1">
          {STEPS.map((step) => (
            <span key={step.key} className="text-[9px] text-slate-400 dark:text-ink-500 text-center" style={{ width: "20%" }}>{step.label}</span>
          ))}
        </div>
      </div>

      {/* Items */}
      <div className="px-3 py-2 border-t border-slate-50 dark:border-ink-700">
        {order.items?.slice(0, 2).map((item, i) => (
          <div key={i} className="flex items-center gap-2 py-1">
            {item.product_image && <img src={item.product_image} className="w-7 h-7 rounded-lg object-cover" alt="" />}
            <p className="text-xs text-slate-600 dark:text-ink-300 truncate flex-1">{item.product_title}</p>
            <p className="text-xs font-bold text-orange-600">${item.price?.toFixed(2)}</p>
          </div>
        ))}
        <div className="flex items-center justify-between mt-1">
          <span className="text-xs font-bold text-slate-800 dark:text-ink-100">Total: ${order.total?.toFixed(2)}</span>
