    ...
    <div className={`flex gap-2 items-end ${isMine ? "flex-row-reverse" : "flex-row"} group relative`}>
      ...
      <div className="relative max-w-[72%]">
        ...
        <AnimatePresence>
          {showReactions && (
            <motion.div
              ...
              className={`absolute ${isMine ? "right-0" : "left-0"} -top-11 flex gap-1 bg-white dark:bg-ink-800 rounded-2xl shadow-xl border border-slate-100 dark:border-ink-700 px-2 py-1.5 z-20`}
            >
              {EMOJI_QUICK.map(e => (
        ...
        <AnimatePresence>
          {showActions && (
            <motion.div
              ...
              className={`absolute ${isMine ? "right-0" : "left-0"} -top-2 translate-y-[-100%] bg-white dark:bg-ink-800 rounded-2xl shadow-xl border border-slate-100 dark:border-ink-700 py-1 z-30 min-w-[130px]`}
            >
              {ACTIONS.map((action) => (
                <button
                  key={action.label}
                  onClick={action.onClick}
                  className={`flex items-center gap-2.5 w-full px-3 py-2 text-xs font-medium hover:bg-slate-50 dark:hover:bg-ink-700 transition-colors ${action.danger ? "text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950" : "text-slate-700 dark:text-ink-300"}`}
                >
        ...
        {editing ? (
          <div className="flex gap-2">
            <input
              ...
              autoFocus
              className="flex-1 px-3 py-2 text-sm rounded-xl border border-orange-300 dark:border-orange-800 outline-none bg-white dark:bg-ink-800 text-slate-900 dark:text-white"
            />
            ...
            <button onClick={() => setEditing(false)} className="px-3 py-1 bg-slate-100 dark:bg-ink-700 text-slate-600 dark:text-ink-300 text-xs rounded-xl">Cancel</button>
          </div>
        ) : (
          <div
            ...
            className={`rounded-2xl text-sm cursor-pointer select-none overflow-hidden ${
              isMine
                ...
                : "bg-white dark:bg-ink-800 text-slate-700 dark:text-ink-200 border border-slate-100 dark:border-ink-700 rounded-bl-sm shadow-sm"
            }`}
          >
            {/* Reply context — inside bubble */}
            {msg.reply_to_content && (
              <div className={`flex items-stretch gap-0 mx-1 mt-1 mb-0 rounded-xl overflow-hidden ${isMine ? "bg-orange-700/50" : "bg-slate-100 dark:bg-ink-700"}`}>
                ...
                <div className="px-2.5 py-1.5 min-w-0">
                  ...
                  <p className={`text-[11px] truncate ${isMine ? "text-orange-100" : "text-slate-500 dark:text-ink-400"}`}>
                    {msg.reply_to_content}
                  </p>
                </div>
              </div>
            ...
            <div className="px-4 py-2.5">
              ...
              {msg.is_edited && <span className={`text-[9px] italic ${isMine ? "text-orange-200" : "text-slate-400 dark:text-ink-500"}`}> (edited)</span>}
              <div className={`flex items-center gap-1 mt-0.5 text-[10px] ${isMine ? "text-orange-200 justify-end" : "text-slate-400 dark:text-ink-500"}`}>
          ...
          <div className={`flex flex-wrap gap-1 mt-1 ${isMine ? "justify-end" : "justify-start"}`}>
            {Object.entries(localReactions).map(([emoji, count]) => count > 0 ? (
              <button
                ...
                className="flex items-center gap-0.5 bg-white dark:bg-ink-700 border border-slate-200 dark:border-ink-600 rounded-full px-1.5 py-0.5 text-xs shadow-sm hover:bg-slate-50 dark:hover:bg-ink-600 transition-colors"
              >
                <span>{emoji}</span>
                <span className="text-[10px] font-semibold text-slate-600 dark:text-ink-300">{count}</span>
              </button>
            ) : null)}
          </div>
        )}
        ...
        <div className={`hidden group-hover:flex items-center gap-1 mt-1 ${isMine ? "justify-end" : "justify-start"}`}>
          <button onClick={() => setShowReactions(v => !v)} className="w-6 h-6 rounded-full bg-slate-100 dark:bg-ink-700 hover:bg-slate-200 dark:hover:bg-ink-600 flex items-center justify-center">
            <Smile className="w-3.5 h-3.5 text-slate-500 dark:text-ink-400" />
          </button>
          <button onClick={() => onReply?.(msg)} className="w-6 h-6 rounded-full bg-slate-100 dark:bg-ink-700 hover:bg-slate-200 dark:hover:bg-ink-600 flex items-center justify-center">
            <Reply className="w-3.5 h-3.5 text-slate-500 dark:text-ink-400" />
          </button>
          <button onClick={() => setShowActions(v => !v)} className="w-6 h-6 rounded-full bg-slate-100 dark:bg-ink-700 hover:bg-slate-200 dark:hover:bg-ink-600 flex items-center justify-center">
            <MoreHorizontal className="w-3.5 h-3.5 text-slate-500 dark:text-ink-400" />
          </button>
        </div>
      </div>
    </div>
